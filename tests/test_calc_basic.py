...
@pytest.mark.asyncio
async def test_bmi_success_generates_cards(monkeypatch):
    monkeypatch.setattr(calc, "session_scope", _dummy_scope)
    monkeypatch.setattr(calc.users_repo, "get_or_create_user", AsyncMock())
    monkeypatch.setattr(calc, "set_last_plan", AsyncMock())
    monkeypatch.setattr(calc.events_repo, "log", AsyncMock())
    ...
    message = _make_message(user_id, "180 80")
...
@pytest.mark.asyncio
async def test_bmi_invalid_input_prompts_retry(monkeypatch):
    monkeypatch.setattr(calc, "session_scope", _dummy_scope)
    monkeypatch.setattr(calc.users_repo, "get_or_create_user", AsyncMock())
    monkeypatch.setattr(calc, "set_last_plan", AsyncMock())
    monkeypatch.setattr(calc.events_repo, "log", AsyncMock())
    ...
    user_id = 404
    calc.SESSIONS[user_id] = {"calc": "bmi"}
    message = _make_message(user_id, "рост вес")

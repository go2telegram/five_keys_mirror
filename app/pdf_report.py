...
def _intake_table(data_rows: list[dict], body_style: ParagraphStyle) -> Table:
    ...
    for r in data_rows:
        table_data.append([
            ...
            mark(r.get("morning")), mark(r.get("day")), mark(r.get("evening")),
...
def build_pdf(
    ...
    title_s = ParagraphStyle("title_s", parent=styles["Title"], fontName=bold_font,
                             textColor=colors.HexColor("#2E7D32"), fontSize=22, leading=26, alignment=1)
    h_s = ParagraphStyle("h_s", parent=styles["Heading2"], fontName=bold_font,
                         textColor=colors.HexColor("#388E3C"), fontSize=14, leading=18)
    p_s = ParagraphStyle(
        "p_s", parent=styles["BodyText"], fontName=reg_font, fontSize=11, leading=15)
    bullet_s = ParagraphStyle("bullet_s", parent=p_s, leftIndent=0.2*cm,
                              bulletFontName=reg_font, bulletFontSize=10)
    ...
    if actions:
        ...
        a_items = [ListItem(Paragraph(a, bullet_s), bulletColor=colors.HexColor(
            "#2E7D32")) for a in actions]
        story.append(ListFlowable(a_items, bulletType="bullet",
                     bulletColor=colors.HexColor("#2E7D32")))
    ...
    story.append(Spacer(1, 0.2*cm))
    pr_items = [ListItem(Paragraph(s, bullet_s), bulletColor=colors.HexColor(
        "#2E7D32")) for s in products]
    story.append(ListFlowable(pr_items, bulletType="bullet",
                 bulletColor=colors.HexColor("#2E7D32")))
    story.append(Spacer(1, 0.5*cm))

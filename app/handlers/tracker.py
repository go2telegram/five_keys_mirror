...
async def _snapshot(
    ...
    *,
    ...
    async with compat_session(session_scope) as session:
        ...
        start_utc, end_utc = day_bounds(today, tz)
        week_start = today - dt.timedelta(days=6)
        week_start_utc, _ = day_bounds(week_start, tz)
        events = await tracker_repo.fetch_events(session, user.id, since=week_start_utc, until=end_utc)

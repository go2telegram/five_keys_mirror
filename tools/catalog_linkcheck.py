...
async def run_checks(
    ...
    *,
    ...
    requester: (
        Callable[[aiohttp.ClientSession, LinkTarget], tuple[int | None, str | None]] | None
    ) = None,
...
async def _async_main(args: argparse.Namespace) -> tuple[int, dict[str, object]]:
    ...
    print(
        f"SUMMARY status={summary['status']} total={total} broken={broken} duration={duration}s log={summary['log']}"
    )

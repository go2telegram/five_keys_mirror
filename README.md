...
За минуту поднимите локальную или продовую среду готовыми сценариями:

- **DRY** — прогоняет все шаги без побочных эффектов: `bash tools/dev_up.sh --dry`.
- **PROD** — запускает контейнер с реальным ботом: `bash tools/dev_up.sh --prod --bot-token="..." --health-port=8080`.
- **Генерация ссылок** — собирает каталожные и партнёрские ссылки: `bash tools/dev_up.sh --links --base-register="..." --dry`.
- **PS** — Windows-эквивалент для PowerShell/CMD: `tools/dev_up.cmd --prod --bot-token=...`.

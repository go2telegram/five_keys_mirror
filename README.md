...
- `/premium_center` — главное меню Premium-центра с доступом к плану, прогрессу и редактору целей.
- `/track_water <мл>` — добавить стакан воды (по умолчанию `+250`).
- `/track_sleep <часы>` — отметить сон за сутки (по умолчанию `7`).
- `/track_stress <0-10>` — оценить уровень стресса (по умолчанию `3`).
- `/progress_chart` — показать график прогресса за 7 дней (вода, сон, стресс).
- `/premium_report` и `/premium_report_img` (только админам) — текстовый отчёт и диаграмма по премиум-метрикам.

План на неделю автоматически обновляется каждую неделю (по расписанию `WEEKLY_PLAN_CRON`) и сохраняется в личном профиле. Любые правки через «🧠 Редактировать цели» фиксируются в истории (`var/plans/<user_id>_*.json`).
...
Перед первым запуском задайте `MIGRATE_ON_START=1`, чтобы Alembic автоматически применил все миграции и база не упала с ошибкой
`no such column`:

```bash
MIGRATE_ON_START=1 bash tools/dev_up.sh --dry
MIGRATE_ON_START=1 bash tools/dev_up.sh --prod --bot-token="123:AA..."
```

Если переменная не указана, `tools/dev_up.sh` проставит её за вас (значение можно переопределить вручную при необходимости).
...
Все проверки и автоформат выполняются автоматически:
- `dev_up.sh` форматирует код при запуске;
- CI (`.github/workflows/ci.yml`) запускает `ruff --fix --unsafe-fixes`;
- при коммите работает pre-commit hook.

Команда для локального форматирования:
...
## Как безопасно подтягиваться

1. Перед `git pull` проверьте, нет ли несохранённых изменений (`git status`). При необходимости временно уберите их в stash (`git stash push -u -m "WIP: local"`).
2. Выполните `git pull --ff-only`, чтобы подтянуть только fast-forward обновления.
3. Верните изменения из stash (`git stash pop`), убедитесь в отсутствии конфликтов и повторите коммит.

Чтобы автоматизировать последовательность, воспользуйтесь скриптом `tools/git_clean.sh`. Он предлагает сохранить изменения в stash и выполняет безопасный `git pull --ff-only`.
...
Комбинация `ruff check . --fix` и `ruff format .` повторяет CI. Для проверки без модификаций используйте `black --check .`.
...
CI автоматически применяет автоисправления к PR: отдельная job прогоняет Ruff ≥ 0.14 (`ruff check . --fix` + `ruff format .`) и коммитит результат (`chore: ruff autofix + format`), если появились изменения. Security-job фейлится только на HIGH/CRITICAL; gitleaks в оффлайне пропускается. Если после автофикса остаются замечания, их нужно устранить вручную.

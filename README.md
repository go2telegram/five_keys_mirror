...
- 🗂️ Собранный каталог сохраняется в `app/catalog/products.json` и доступен в боте по `/catalog_report`.
...
Импортёр сохраняет итоговый каталог только в `app/catalog/products.json`. Если какое-то описание или изображение не сопоставлено,
в лог попадает предупреждение `WARNING: No image for …`, а продукт получает `available=false`. Отдельного отчёта сборщик не создаёт: чтобы убедиться, что каталог полный, проверьте лог и содержимое `products.json`. Команда `validate` лишь подтверждает структуру каталога.

### Отчётность и контроль «38/38»

После успешной сборки проверьте отчёт:

- локально откройте `app/catalog/products.json` и убедитесь, что продукты содержат изображения и `available=true`; проверьте логи
  сборки на предмет предупреждений `No image for …`;
...
Посмотрите предупреждения `No image for …` в логе сборки. Убедитесь, что файлы попадают в нужные каталоги, названы по slug продукта и имеют одно из поддерживаемых расширений. После исправлений перезапустите сборку.

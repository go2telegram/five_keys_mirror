...
- Команда `/register` запускает анкету, а `/profile` позволяет уточнить предпочтения позднее; оба сценария доступны из главного меню.
- Предпочтения сохраняются в таблице `user_profiles` (миграция `alembic/versions/0003_add_user_profiles.py`). Перед запуском
  примените `alembic upgrade head`, чтобы схема БД включала модель `UserProfile`.
- Калькуляторы, подбор продукта и квизы учитывают сохранённые ответы, поэтому пользователи получают персонализированные рекомендации.

## Главное меню и помощь

- Главное меню содержит быстрые кнопки: «Каталог», «Тесты», «Калькуляторы», «Подбор продукта», «Профиль», «Подписка» и «Помощь».
- Команда `/help` открывает inline-FAQ. Контент хранится в `app/faq/faq.yaml`; обновите YAML, чтобы поменять вопросы и ответы.

## Администрирование

- `/admin/doctor` показывает состояние каталога, версию базы данных и текущую сборку, помогая быстро диагностировать окружение.
- `/admin/analytics` формирует сводку по квизам, калькуляторам и CTR рекомендаций.

...
Бот собирает события поведения пользователей и использует их в отчётности и админ-панели.

- События пишутся через `app.services.analytics.log_event` (поддерживаются `quiz_started`, `quiz_finished`, `calc_done`, `reco_shown`,
  `reco_click_buy`, `order_paid`, `premium_on`, `premium_off` и другие бизнес-события).
- Ежедневный экспорт сохраняет CSV в `/var/exports/events_YYYYMMDD.csv` и может параллельно отправлять данные в Google Sheets
  (`GOOGLE_SHEET_ID`) и ClickHouse (`CLICKHOUSE_URL`).
- Для ручного запуска используйте `python tools/export_events.py` или выберите отчёт из `python tools/reports.py`.
- В админ-чате доступны команды `/funnel_report`, `/cohort_report`, `/ctr_report`.
- Простая панель `/admin/dashboard` (FastAPI) показывает воронку, ретеншн и CTR; доступ контролируется админ-токеном.

> 💡 Scheduler автоматически рассылает дайджест в админский чат: убедитесь, что задан `ANALYTICS_ADMIN_CHAT_ID` (или `ADMIN_ID`).

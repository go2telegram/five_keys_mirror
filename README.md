...
Импортёр пишет результат в `app/catalog/products.json`. Если какое-то описание или изображение не сопоставлено, в лог попадает
предупреждение `WARNING: No image for …`, а продукт получает `available=false`. Команда `validate` подтверждает структуру каталога.
